...
from geodude.utils import flatten_geom_collection
...
class TestMergeGeoms:
    ...
    def test_merge_geoms_different_geometry_types(self):
        ...
        point = Point(0, 0)
        merged = flatten_geom_collection([poly, line, point])
    ...
    def test_merge_geoms_empty_list(self):
        ...
        merged = flatten_geom_collection([])
        assert len(merged) == 0
    ...
    def test_merge_geoms_single_geometry(self):
        ...
        result = flatten_geom_collection(geom)
    ...
    def test_merge_geoms_sub_geometries(self):
        ...
        geom4 = [geom1, geom2]
        geoms = [geom4, geom3]
        result = flatten_geom_collection(geoms)
        assert isinstance(result, gpd.GeoDataFrame)
        assert len(result) == 3
    ...
    def test_merge_geoms_return_type(self):
        ...
        geom2 = Polygon(((2, 2), (2, 3), (3, 3), (3, 2), (2, 2)))
        geoms = [geom1, geom2]
        result_gdf = flatten_geom_collection(geoms, as_gdf=True)
        result_list = flatten_geom_collection(geoms, as_gdf=False)
