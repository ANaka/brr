    ...
    for i in range(number_of_lines):
        if i % 2:
            coords.extend(
                [
                    (
                        (centre_x - hatch_length / 2, centre_y - hatch_length / 2 + i * spacing),
                        (centre_x + hatch_length / 2, centre_y - hatch_length / 2 + i * spacing),
                    )
                ]
            )
...
def add_jittered_midpoints(ls, n_midpoints, xstd, ystd, xbias=0, ybias=0):
    eval_range = np.linspace(0.0, 1.0, n_midpoints + 2)
...
def LineString_to_jittered_bezier(
    ls, n_midpoints=1, xbias=0.0, xstd=0.0, ybias=0.0, ystd=0.0, normalized=True, n_eval_points=50
):
    if normalized == True:
...
class RecursiveCirclePacker(object):
    ...
    def gen_relative_rad_seq(self):
        return np.linspace(self.fill_poly_rad * self.rad_seq_start, self.fill_poly_rad * self.rad_seq_end, self.n_rads)

    def gen_absolute_rad_seq(self):
        return np.linspace(self.rad_seq_start, self.rad_seq_end, self.n_rads)
...
def gaussian_random_walk(n, step_init=1, step_mu=0.0, step_std=1, scale=True):
    ys = []
    y = step_init
    if scale:
        step_mu /= n
...
class DelauneyLattice(object):
    ...
    def _buffer_func(self):
        return functools.partial(self._buffer, cap_style=self._cs, join_style=self._js)
...
class AffineMatrix(DataClassBase):
    ...
    def rotation_matrix(self):
        a = self.angle
        return np.array([[np.cos(a), -np.sin(a), 0.0], [np.sin(a), np.cos(a), 0.0], [0, 0, 1]])

    @property
    def translation_matrix(self):
        return np.array([[1, 0, -self.xoff], [0, 1, -self.yoff], [0, 0, 1]])

    @property
    def scaling_matrix(self):
        return np.array([[self.xfact, 0, 1], [0, self.yfact, 1], [0, 0, 1]])
...
class StellarSnowflake(object):
    ...
    radius_f0: float = 1.0
    n_corners_f0: int = 6
    rotation_f0: float = 0.0  # degrees
    radius_f1: float = 0.3
    n_corners_f1: int = 6
    rotation_f1: float = 0.0  # degrees

    @property
    def f0(self):
        return RegPolygon(self.point_f0, self.radius_f0, self.n_corners_f0, self.rotation_f0)
...
def recursive_split(
    poly, split_func=random_line_subdivide, p_continue=0.7, depth=0, depth_limit=15, buffer_kwargs=None
):
...
def recursive_split_frac_buffer(
    poly,
    split_func=random_line_subdivide,
    p_continue=0.7,
    depth=0,
    depth_limit=15,
    buffer_kwargs=None,
...
def vsketch_to_shapely(sketch):
    return [
        [LineString([Point(pt.real, pt.imag) for pt in lc]) for lc in layer]
        for layer in sketch.document.layers.values()
    ]
