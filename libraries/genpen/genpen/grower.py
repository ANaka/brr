...
from genpen import utils as utils
...
class GrowerParams(object):
    ...
    def reg_poly(self, pt):
        ...
        new_row["geometry"] = gp.RegPolygon(
            pt["geometry"], n_corners=self.get_n_corners(), rotation=self.get_rotation(), radius=self.get_rad()
        ).poly
        return new_row
...
class Grower(object):
    ...
    def get_random_boundary_pts(self, n_pts=1):
        self.new_pts = geopandas.GeoDataFrame()
        self.new_pts["geometry"] = [
            self.agg_poly.boundary.interpolate(d, normalized=True) for d in self._p.boundary_pt_dist.rvs(n_pts)
        ]
